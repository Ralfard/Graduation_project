<main class="main article-feed">
    <div class="main__wrapper">
        ...
        $sqlGetBookmarks = "SELECT bookmarks FROM users__bookmarks WHERE user_id=" . $_SESSION['user']['id'] . "";
        $requestGetBookmarks = $mysqli->query($sqlGetBookmarks);
        $bookmarks = $requestGetBookmarks->fetch_assoc();
        $bookmarks=$bookmarks['bookmarks'];
        $bookmarks=unserialize($bookmarks);
        $bookmarks=implode(',', $bookmarks);

        $sqlArticle = "SELECT * FROM `articles`WHERE `id` IN ($bookmarks)";
