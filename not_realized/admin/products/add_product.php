<?php
session_start();
if ($_SESSION['auth_admin'] == 'yes_auth') {
    define("sunwa-efimov-.r.a", true); //запрещает переход по ссылкам через путь в адресной строке
    if (isset($_GET['logout'])) {
        unset($_SESSION['auth_admin']);
        header('Location: index.php');
    }
}
include("../../include/db_connect.php");

if ($_POST["submit_enter"]) {
    $error = array();
    if (count($error)) { //                    объединяем массив ошибок в одну строку
        $_SESSION["message"] = "<p id='form-error>" . implode('<br>', $error) . "</p>";
    } else {
        $name = $_POST['name'];
        $price = $_POST['price'];
        $discount = $_POST['discount'];
        $article_number = $_POST['article_number'];
        $manufacturer = $_POST['manufacturer'];
        $description = $_POST['description'];
        $terminal = $_POST['terminal'];
        $power = $_POST['power'];
        $input_voltage = $_POST['input_voltage'];
        $output_voltage = $_POST['output_voltage'];
        $rated_current = $_POST['rated_current'];
        $intagration = $_POST['intagration'];


        $query = "INSERT INTO products (name, price, discount, article_number, manufacturer, terminal, power, input_voltage, output_voltage, rated_current, intagration, description) VALUES
        (
            '" . mysqli_real_escape_string($mysqli, $name) . "',
            '" . mysqli_real_escape_string($mysqli, $price) . "',
            '" . mysqli_real_escape_string($mysqli, $discount) . "',
            '" . mysqli_real_escape_string($mysqli, $article_number) . "',
            '" . mysqli_real_escape_string($mysqli, $manufacturer) . "',
            '" . mysqli_real_escape_string($mysqli, $terminal) . "',
            '" . mysqli_real_escape_string($mysqli, $power) . "',
            '" . mysqli_real_escape_string($mysqli, $input_voltage) . "',
            '" . mysqli_real_escape_string($mysqli, $output_voltage) . "',
            '" . mysqli_real_escape_string($mysqli, $rated_current) . "',
            '" . mysqli_real_escape_string($mysqli, $intagration) . "',
            '" . mysqli_real_escape_string($mysqli, $description) . "'
        )";
        $result = mysqli_query($mysqli, $query) or die("Ошибка " . mysqli_error($mysqli));

        $id = mysqli_insert_id($mysqli);

        if (empty($_POST['galleryimg[]'])) {
            include("../action/product_gallery.php");
            unset($_POST["galleryimg[]"]);
        }
    }
}
?>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>

    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><!-- шрифты -->
    <link href="https://fonts.googleapis.com/css2?family=Roboto&display=swap" rel="stylesheet">
    <link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet"> <!-- иконки-->
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@500&display=swap" rel="stylesheet">

    <link rel="stylesheet" href="../../styles/reset.css">
    <link rel="stylesheet" href="../../styles/grid.css">
    <link rel="stylesheet" href="../styles/common_styles.css">
    <link rel="stylesheet" href="../styles/admins.css">
</head>

<body>
    <?php
    include_once("../include/header.php");
    $error;
    ?>


    <section class="main">
        <div class="container">
            <div class="row main__row">
                <h2 class="main__title">Добавление Товара</h2>
                <a href="<?php echo $_SERVER['REQUEST_SCHEME']; ?>://<?php print($_SERVER['HTTP_HOST']); ?>/admin/products"><button class="btn">Назад</button></a>
            </div>
            <hr>
            <div class="row">
                <form enctype="multipart/form-data" class="form" method="post" action="">
                    <div class="form__block">
                        <label class="form__label" for="">Название</label>
                        <input name="name" type="text" required class="form__input">
                    </div>
                    <div class="form__block">
                        <label class="form__label" for="">Цена</label>
                        <input name="price" type="text" required class="form__input">
                    </div>
                    <div class="form__block">
                        <label class="form__label" for="">Скидка</label>
                        <input name="discount" type="text" required class="form__input">
                    </div>
                    <div class="form__block">
                        <label class="form__label" for="">Артикул</label>
                        <input name="article_number" type="text" required class="form__input">
                    </div>
                    <div class="form__block">
                        <label class="form__label" for="">Текст Для выдачи</label>
                        <textarea name="description" required class="form__input"></textarea>
                    </div>

                    <div class="form__block">
                        <label class="form__label" for="">Производитель</label>
                        <select class="form__select" required name="manufacturer" id="">
                            <option value='Omron' class="form__option">Omron</option>
                            <option value='Nichicon' class="form__option">Nichicon</option>
                            <option value='Sato' class="form__option">Sato</option>
                            <option value='Stanley' class="form__option">Stanley</option>
                            <option value='Weintek' class="form__option">Weintek</option>
                        </select>
                    </div>


                    <div class="form__block">
                        <label class="form__label" for="">Наличие терминала</label>
                        <input name="terminal" type="text" required class="form__input">
                    </div>
                    <div class="form__block">
                        <label class="form__label" for="">Мощность</label>
                        <input name="power" type="text" required class="form__input">
                    </div>
                    <div class="form__block">
                        <label class="form__label" for="">Входное напряжение</label>
                        <input name="input_voltage" type="text" required class="form__input">
                    </div>
                    <div class="form__block">
                        <label class="form__label" for="">Выходное напряжение</label>
                        <input name="output_voltage" type="text" required class="form__input">
                    </div>
                    <div class="form__block">
                        <label class="form__label" for="">Номинальный ток</label>
                        <input name="rated_current" type="text" required class="form__input">
                    </div>
                    <div class="form__block">
                        <label class="form__label" for="">Интеграция</label>
                        <input name="intagration" type="text" required class="form__input">
                    </div>


                    <div class="form__block">
                        <label for="" class="form__label">Галлерея картинок</label>
                        <div id="objects">
                            <div id="addimage1" class="addimage">
                                <input type="hidden" name="MAX_FILE_SIZE" value="5000000">
                                <input type="file" class="form-control" name="galleryimg[]"><br>
                            </div>
                        </div>
                    </div>

                    <p class="form__btn" id="add-input">Добавить картинку</p>
                    <br>

                    <div class="form__block">
                        <input name="submit_enter" type="submit" class="btn" value="Создать">
                    </div>
                </form>
            </div>
        </div>

    </section>


    <script src="../js/jquery-3.6.0.min.js"></script>
    <script src="../js/main.js"></script>
</body>

</html>